...
static inline void foo(const __m256i &pattern, const __m256i *mem, size_t offset, size_t *&tokens)
{
...
void tokenize(const char *buffer, size_t buffer_size, size_t *tokens, size_t token_size)
{
    ...
    {
        foo(pattern, reinterpret_cast<const __m256i *>(buffer + i), i, tokens);
        foo(pattern, reinterpret_cast<const __m256i *>(buffer + i + 1), i + 1, tokens);
    }
}
