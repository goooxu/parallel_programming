...
void test(const char *buffer, size_t buffer_size, size_t *breaks,
  ...
  size_t d_buffer_padding_size =
      1 + (buffer_size % 2048 != 0) ? (2048 - buffer_size % 2048) : 0;
  ...
  cudaMalloc((void **)&d_buffer, buffer_size + d_buffer_padding_size);
  ...
  if (d_buffer_padding_size != 0) {
    cudaMemset((void *)(d_buffer + buffer_size), 0, d_buffer_padding_size);
  }
  ...
  foo<<<blocks, block_dim>>>(d_buffer, buffer_size + d_buffer_padding_size,
                             d_breaks, d_break_index);
  cudaDeviceSynchronize();
